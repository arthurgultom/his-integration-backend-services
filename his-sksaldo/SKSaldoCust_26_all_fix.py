...
conn22      = pymysql.connect(host='10.152.9.94',user='root',password='EuXbrmzvVBjDSB99',database='his_db_final_3_dev')
...
def insertLogSend(dealerName) :
    ...
    conLog      = pymysql.connect(host='10.152.9.94',user='root',password='EuXbrmzvVBjDSB99',database='his_db_final_3_dev')
    ...
    curhisLog.execute(""" 
        ...
        values('','','"""+str(dealerName)+"""', '', '"""+str(mth)+"""', '"""+str(yth)+"""','0', '"""+str(mydates)+"""', '' )
    """)
    curhisLog.close()
...
def updateSendMail(dealerName) :
    conUpdate       = pymysql.connect(host='10.152.9.94',user='root',password='EuXbrmzvVBjDSB99',database='his_db_final_3_dev')
...
def checkFlagSendMail() :
    ...
    conFlag     = pymysql.connect(host='10.152.9.94',user='root',password='EuXbrmzvVBjDSB99',database='his_db_final_3_dev')
...
def ProsesSendMail(dealerName, regional) :
    ...
    conn2       = pymysql.connect(host='10.152.9.94',user='root',password='EuXbrmzvVBjDSB99',database='his_db_final_3_dev')
    ...
    for z in range(0,x):
        ...
        """
            ...
            smtp = smtplib.SMTP("10.17.51.22", 587)
            # smtp.starttls()
...
try:
    ...
    for rowt in curhisxx:
        
        # try:
        
        insertLogSend(rowt[0])
        ProsesSendMail(rowt[0], rowt[2])
            
        # except:
        
            # checkFlagSendMail()
    
        # get list data dealer
