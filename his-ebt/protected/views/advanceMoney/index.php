...
?>
<?php
if (Yii::app()->user->hasFlash('message_diff')): ?>
    <div class="alert alert-danger">
        <?php echo Yii::app()->user->getFlash('message_diff'); ?>
    </div>
<?php endif; ?>

<?php if ($this->isAllowed(Yii::app()->user->id, 'index')) { ?>

    <div class="page-header">
        <h3><?php echo Yii::t('menu', 'sAdvancedMoney'); ?> - <?php echo Yii::t('menu', 'subList'); ?></h3>
    </div>

    <div style="width: 100%;text-align:right">
        <?php
        if (Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) == 20) {
            echo CHtml::link('<i class="fa fa-print"></i>&nbsp;&nbsp;Export Excel', array('advancemoney/export'), array(
                'class' => 'btn btn-warning btn-sm',
            ));
        }
        if (Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) == 58) {
            echo CHtml::link('<i class="fa fa-print"></i>&nbsp;&nbsp;Export Excel', array('advancemoney/exportIAD'), array(
                'class' => 'btn btn-warning btn-sm',
            ));
        }
        ?>
    ...
    <div style="overflow-x:auto; width: 100%; white-space: nowrap;">
        ...
        $this->widget('zii.widgets.grid.CGridView', array(
            'id' => 'advance-money-model-grid',
            'dataProvider' => $model->search_filter_by_emp(Yii::app()->user->id),
            'filter' => $model,
            ...
            'columns' => array(
                array(
                    'class' => 'CButtonColumn',
                    'htmlOptions' => array('width' => '2%', 'valign' => 'middle'),
                    'template' => '{view}&nbsp;{update}&nbsp;{delete}',
                    'buttons' => array(
                        'view' => array(
                            'imageUrl' => false,
                            'label' => '  View',
                            'options' => array('class' => 'fa fa-search btn btn-info btn-xs'),
                            //'icon'=>'fa fa-plus',
                        ),
                        'update' => array(
                            'imageUrl' => false,
                            ...
                            'visible' => '($data->status != 2 && Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) != 20 && Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) != 58)',
                            //'icon'=>'fa fa-plus',
                        ),
                        'delete' => array(
                            'imageUrl' => false,
                            ...
                            'visible' => '($data->rel_fin_pum->approver_flag != 2 && $data->status == 2 && Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) != 20 && Yii::app()->globalFunction->get_user_role_id(Yii::app()->user->id) != 58)',
                            //'icon'=>'fa fa-plus',
                        ),
                    ),
                ),
