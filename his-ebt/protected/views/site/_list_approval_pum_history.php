...
$this->breadcrumbs = array(
    ...
    Yii::t('menu','subHistory'),
);
?>

<div class="page-header"><h3><?php echo Yii::t('menu','sAdvancedMoney'); ?> - <?php echo Yii::t('menu','subHistory');?></h3></div>

<div style="width:100%;text-align:right;">
    <?php if ($role == 25 || $role == 24) {
    echo CHtml::link('<i class="fa fa-print"></i>&nbsp;&nbsp;Export Excel',array('advancemoney/export'), array('class'=>'btn btn-warning btn-sm'), array('id' => $model->adv_mon_id));
    ?>
    &nbsp;&nbsp;
    <?php
    echo CHtml::link('<i class="fa fa-send"></i>&nbsp;Send to Outstander', array('advancemoney/SendToOutstanding'), array('class' => 'btn btn-info btn-sm ')); 
    ?>
    <?php } ?>
</div>
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    'dataProvider' => ($role == 25 || $role == 24) ? $model->searchHistoryFAD(Yii::app()->user->id) : $model->searchApprovalHistory(Yii::app()->user->id),
    'filter' => $model,
	    ...
	    array(
            ...
            'buttons' => array(
                'view' => array(
                    ...
                    'url'=>'Yii::app()->createUrl("advancemoney/view", array("id"=>$data["adv_mon_id"]))'
                ),
                'update' => array(
                    'imageUrl' => false,
                    'label' => '  Edit',
                    'options' => array('class' => 'fa fa-edit btn btn-warning btn-xs'),
                    'url'=>'Yii::app()->createUrl("advancemoney/update", array("id"=>$data["adv_mon_id"]))',
                    'visible'=> function ($url, $model, $key) use ($role) {
                        return ($model->status == 2 and $model->paid_status == 0 and ($role == 24 or $role == 25)) ? true : false;
                    }
                ),
                'delete' => array(
                    'imageUrl' => false,
                    ...
                    'url'=>'Yii::app()->createUrl("advancemoney/delete", array("id"=>$data["adv_mon_id"]))',
